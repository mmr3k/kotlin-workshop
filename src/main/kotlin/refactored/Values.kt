...
data class Money(val value: BigDecimal, val currency: Currency) {
    operator fun plus(other: Money): Money {
        check(currency == other.currency) {"Can't add two Money values. Currencies are different."}
        return Money(value.add(other.value), currency)
    }


    operator fun minus(other: Money): Money {
        check(currency == other.currency) {"Can't subtract two Money values. Currencies are different."}
